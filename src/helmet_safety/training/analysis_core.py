...
def validated_streaming_image_source(images_dir: Path, *, expected_images: int) -> str:
    """Validate an image directory, then return it as a lazy predictor source."""
    ...
    if actual != expected_images:
        raise RuntimeError(f"image count mismatch: {actual} != {expected_images}")
    return source
...
def summarize_fixed_threshold_detections(
    records: Sequence[Mapping[str, object]], *, iou_threshold: float = 0.5
) -> dict[str, object]:
    """Summarize fixed-confidence class-aware detections overall and per class."""

    if not 0 < iou_threshold <= 1:
        raise ValueError("IoU threshold must be within (0, 1]")
    counts = {
        class_id: {"ground_truth": 0, "predictions": 0, "tp": 0, "fn": 0, "fp": 0}
        for class_id in (0, 1)
    }
    for record in records:
        ground_truth = record["ground_truth"]  # type: ignore[assignment]
        predictions = record["predictions"]  # type: ignore[assignment]
        matches = _class_aware_match_pairs(ground_truth, predictions, iou_threshold=iou_threshold)
        ...
        for gt_index, gt in enumerate(ground_truth):
            ...
            counts[class_id]["ground_truth"] += 1
        ...
        for prediction_index, prediction in enumerate(predictions):
            ...
            counts[class_id]["predictions"] += 1
            if prediction_index not in matched_predictions:
                counts[class_id]["fp"] += 1

    images = len(records)

    def metric_row(values: Mapping[str, int], *, include_images: bool = False) -> dict[str, object]:
        tp, fn, fp = int(values["tp"]), int(values["fn"]), int(values["fp"])
        precision = tp / (tp + fp) if tp + fp else None
        recall = tp / (tp + fn) if tp + fn else None
        f1 = 2 * tp / (2 * tp + fp + fn) if 2 * tp + fp + fn else None
        row: dict[str, object] = {
            "ground_truth": int(values["ground_truth"]),
    ...
    overall_counts = {
        key: sum(int(counts[class_id][key]) for class_id in (0, 1))
        for key in ("ground_truth", "predictions", "tp", "fn", "fp")
    }
    return {
        "matching": {"class_aware": True, "iou_threshold": iou_threshold},
        "overall": metric_row(overall_counts, include_images=True),
        "per_class": {
            "helmet": metric_row(counts[0]),
            "no_helmet": metric_row(counts[1]),
        },
    }
...
def summarize_p0_records(
    records: Sequence[Mapping[str, object]], *, matching_iou: float = 0.5, duplicate_iou: float = 0.7
) -> dict[str, object]:
    """Compose full-val fixed-threshold, tiny, and duplicate metrics for P0."""

    return {
        "fixed_threshold_metrics": summarize_fixed_threshold_detections(records, iou_threshold=matching_iou),
        "tiny_metrics": summarize_tiny_ground_truth(records, iou_threshold=matching_iou)["summary"],
        "duplicate_audit": audit_obvious_duplicate_boxes(
            records, duplicate_iou_threshold=duplicate_iou
        ),
    }
