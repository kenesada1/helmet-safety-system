...
TRAINING_REPORT = OUTPUT_DIR / "e8_training_report.json"
EVALUATION_REPORT = OUTPUT_DIR / "e4_e6_e7_e8_full_val_comparison.json"
...
def read_history(results_csv: Path = RESULTS_CSV) -> tuple[list[dict[str, str]], int]:
    with results_csv.open(encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if not rows:
        raise RuntimeError("cannot resume E8 without completed epochs in results.csv")
    best = max(rows, key=lambda row: float(row["metrics/mAP50-95(B)"]))
    return rows, int(best["epoch"])


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Resume the interrupted E8 run from its exact last checkpoint.")
...
def main() -> int:
    ...
    for forbidden in (TRAINING_REPORT, EVALUATION_REPORT):
        if forbidden.exists():
            raise FileExistsError(f"refusing to resume a completed E8 pipeline: {forbidden.resolve()}")
    ...
    with CONSOLE_LOG.open("a", encoding="utf-8", buffering=1) as log_handle:
        ...
        try:
            print(f"\nE8_RESUMED_AT_UTC={resumed_at}", flush=True)
            print(json.dumps(plan, ensure_ascii=False, indent=2), flush=True)
            ...
            if strides != [4.0, 8.0, 16.0, 32.0]:
                raise RuntimeError(f"resume checkpoint lost E8 detection strides: {strides}")
            if not isinstance(model.model.model[E8_REFINEMENT_INDEX], NoECAFeatureRefinement):
                raise RuntimeError("resume checkpoint lost project-local no-ECA refinement module")
            model.add_callback(
                "on_train_start",
                partial(
                    restore_early_stopping,
            ...
            report = {
                ...
                "outputs": {
                    ...
                    "console_log": str((OUTPUT_DIR / "e8_training.log").resolve()),
                },
